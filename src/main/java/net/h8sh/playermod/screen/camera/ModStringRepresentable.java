...
public interface ModStringRepresentable {
    int PRE_BUILT_MAP_THRESHOLD = 16;

    String getSerializedName();

    static <E extends Enum<E> & ModStringRepresentable> ModStringRepresentable.EnumCodec<E> fromEnum(Supplier<E[]> pElementsSupplier) {
        return fromEnumWithMapping(pElementsSupplier, (p_275327_) -> {
            return p_275327_;
        });
    }

    static <E extends Enum<E> & ModStringRepresentable> ModStringRepresentable.EnumCodec<E> fromEnumWithMapping(Supplier<E[]> pEnumValues, Function<String, String> pKeyFunction) {
        ...
        if (ae.length > 16) {
            ...
            return new ModStringRepresentable.EnumCodec<>(ae, (p_216438_) -> {
                return (E)(p_216438_ == null ? null : map.get(p_216438_));
            });
        } else {
            return new ModStringRepresentable.EnumCodec<>(ae, (p_274908_) -> {
                for(E e : ae) {
    ...
    static Keyable keys(final ModStringRepresentable[] pSerializables) {
        return new Keyable() {
            public <T> Stream<T> keys(DynamicOps<T> p_184758_) {
                return Arrays.stream(pSerializables).map(ModStringRepresentable::getSerializedName).map(p_184758_::createString);
            }
        };
    }
    ...
    /** @deprecated */
    @Deprecated
    public static class EnumCodec<E extends Enum<E> & ModStringRepresentable> implements Codec<E> {

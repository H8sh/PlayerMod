...
public class ChargeBarOverlay {
    ...
    public static void setCurrentProgress(float currentProgress) {
        ChargeBarOverlay.currentProgress = currentProgress;
    }
    ...
    public static final IGuiOverlay HUD_CHARGE_BAR_PROGRESS = (ForgeGui gui, GuiGraphics pGuiGraphics, float partialTick, int screenWidth, int screenHeight) -> {
        int i = pGuiGraphics.guiWidth();
        long k = Util.getMillis();
        ChargeBarOverlay.fadeInStart = k;
        ...
        double d0 = d1 * 4.0D;
        int j1 = (int) (d0 * 0.5D);
        float f = ChargeBarOverlay.fadeOutStart > -1L ? (float) (k - ChargeBarOverlay.fadeOutStart) / 1000.0F : -1.0F;
        float f1 = ChargeBarOverlay.fadeInStart > -1L ? (float) (k - ChargeBarOverlay.fadeInStart) / 500.0F : -1.0F;
        ChargeBarOverlay.currentProgress = ChargeBarOverlay.currentProgress++ == ChargeBarOverlay.currentProgressD ? ChargeBarOverlay.currentProgressD: ChargeBarOverlay.currentProgress++;
        if (f < 1.0F) {
            ChargeBarOverlay.drawProgressBar(pGuiGraphics, i / 2 - j1 + 32, k1 - 1, i / 2 + j1 - 32, k1 + 3, 1.0F - Mth.clamp(f, 0.0F, 1.0F));
        }
        if (ChargeBarOverlay.fadeOutStart == -1L && (!ChargeBarOverlay.fadeIn || f1 >= 2.0F)) {
            ChargeBarOverlay.fadeOutStart = Util.getMillis();
        }
    };

    private static void drawProgressBar(GuiGraphics pGuiGraphics, int pMinX, int pMinY, int pMaxX, int pMaxY, float pPartialTick) {
        int i = Mth.ceil((float) (pMaxX - pMinX) * ChargeBarOverlay.currentProgress/ ChargeBarOverlay.currentProgressD);
        int j = Math.round(pPartialTick * 255.0F);
        int k = FastColor.ARGB32.color(j, 0, 0, 0);
        int q = FastColor.ARGB32.color(j, 150, 0, 0);
